...
import { BookOpen, CalendarDays, Check, ChevronDown, CircleDollarSign, Download, FileText, HandCoins, HardHat, Lock, Menu, Plus, Printer, ReceiptIndianRupee, Search, Settings, Trash2, Users, X } from 'lucide-react';
...
function LabourView({ labourers, query, setQuery, onAddLabourer, onDeleteLabourer, onReload, onNotify, unlockWeek, onRecordPayment, onMarkPaid, onDeletePayment }: { labourers: LabourerWithPayments[]; query: string; setQuery: (value: string) => void; onAddLabourer: () => void; onDeleteLabourer: (labourer: Labourer) => void; onReload: () => void; onNotify: (msg: string) => void; unlockWeek: (record: AttendanceRecord) => void; onRecordPayment: (payment: LabourPayment, amount: number) => void; onMarkPaid: (payment: LabourPayment) => void; onDeletePayment: (payment: LabourPayment) => void }) {
  ...
  return <section><PageHeading title="Labour" description="Track workers, attendance and weekly wage payments." action="Add labourer" onAction={onAddLabourer} />
    <div className="labour-summary">
      <div className="report-card"><span>Total wages</span><strong>{money(totalWages)}</strong><small>{allPayments.length} weekly payments</small></div>
      <div className="report-card"><span>Fully paid</span><strong className="positive">{money(totalPaid)}</strong><small>{fullPaidCount} weeks settled</small></div>
      <div className="report-card"><span>Partial paid</span><strong className="info-text">{money(allPayments.reduce((s, p) => s + (p.payment_status === 'Partial' ? Number(p.amount_paid || 0) : 0), 0))}</strong><small>{partialCount} weeks partial</small></div>
      <div className="report-card"><span>Balance due</span><strong className="warning-text">{money(totalUnpaid)}</strong><small>{unpaidCount + partialCount} weeks pending</small></div>
      <div className="report-card"><span>Days worked ({monthLabel})</span><strong>{monthDaysAll}</strong><small>Across all workers</small></div>
    </div>
    <div className="toolbar"><div className="search-box"><Search size={17} /><input placeholder="Search worker name or role" value={query} onChange={(event) => setQuery(event.target.value)} /></div></div>
    <div className="labourer-grid">{filtered.map((labourer) => { const s = workerStats(labourer); const pendingAdvance = labourer.advances.reduce((sum, a) => sum + Number(a.amount_remaining || 0), 0); return <div className="labourer-card" key={labourer.id} onClick={() => setDetailFor(labourer.id)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setDetailFor(labourer.id); } }}>
      <div className="lc-header"><div className="person-cell"><div className="person-avatar"><HardHat size={18} /></div><div><strong>{labourer.name}</strong><small>{labourer.role || 'Worker'}</small></div></div>{s.unpaidTotal > 0 ? <span className="chip due">{money(s.unpaidTotal)} due</span> : <span className="chip settled"><Check size={14} /> Settled</span>}</div>
      <div className="lc-rate"><span>{money(labourer.daily_wage)}/day</span><span>{labourer.phone || 'No phone'}</span></div>
      {pendingAdvance > 0 && <div className="lc-advance-badge"><HandCoins size={14} /> Advance pending: {money(pendingAdvance)}</div>}
      <div className="lc-stats">
        <div className="lc-stat"><span>This month</span><strong>{s.monthDays}</strong><small>{money(s.monthWage)}</small></div>
        <div className="lc-stat"><span>This year</span><strong>{s.yearDays}</strong><small>{money(s.yearWage)}</small></div>
        <div className="lc-stat"><span>Paid out</span><strong className="positive">{money(s.paidTotal)}</strong><small>Full</small></div>
        <div className="lc-stat"><span>Balance</span><strong className={s.unpaidTotal > 0 ? 'warning-text' : 'positive'}>{money(s.unpaidTotal)}</strong><small>{s.unpaidTotal > 0 ? 'Due' : 'Clear'}</small></div>
      </div>
      <button className="button secondary lc-give-advance" onClick={(e) => { e.stopPropagation(); setAdvanceFor(labourer.id); }}><HandCoins size={16} /> Give advance</button>
    </div>; })}</div>
    {filtered.length === 0 && <div className="table-card"><EmptyState title="No workers found" text="Add a labourer to begin weekly attendance." /></div>}
    ...
    {advanceFor && (() => { const l = labourers.find((x) => x.id === advanceFor); if (!l) return null; return <Modal title="Give advance" onClose={() => setAdvanceFor(null)}><AdvanceForm labourer={l} onSubmit={(amount, date, note) => void giveAdvance(l, amount, date, note)} /></Modal>; })()}
  </section>;
}
...
function LabourerDetail({ labourer, weekEnding, setWeekEnding, currentAttendance, toggleDay, updateDayDetail, settleWeek, onRecordPayment, onMarkPaid, onDeletePayment, onUnlockWeek, onDeleteLabourer, onGiveAdvance, onDeleteAdvance, onEditAdvanceNote }: { labourer: LabourerWithPayments; weekEnding: string; setWeekEnding: (v: string) => void; currentAttendance: (l: LabourerWithPayments) => AttendanceRecord | undefined; toggleDay: (l: LabourerWithPayments, d: AttendanceDayKey) => void; updateDayDetail: (l: LabourerWithPayments, d: AttendanceDayKey, field: 'incentive' | 'site', value: number | string) => void; settleWeek: (l: LabourerWithPayments, markPaid: boolean) => void; onRecordPayment: (payment: LabourPayment, amount: number) => void; onMarkPaid: (payment: LabourPayment) => void; onDeletePayment: (payment: LabourPayment) => void; onUnlockWeek: (record: AttendanceRecord) => void; onDeleteLabourer: (labourer: Labourer) => void; onGiveAdvance: (amount: number, date: string, note: string) => void; onDeleteAdvance: (advance: LabourAdvance) => void; onEditAdvanceNote: (advance: LabourAdvance, note: string) => void }) {
  ...
  return <div className="labourer-detail">
    <div className="ld-header"><div className="person-cell"><div className="person-avatar large"><HardHat size={22} /></div><div><h3>{labourer.name}</h3><small>{labourer.role || 'Worker'} · {money(labourer.daily_wage)}/day · {labourer.phone || 'No phone'}</small></div></div><div className="ld-header-actions"><button className="icon-button danger" onClick={() => void onDeleteLabourer(labourer)}><Trash2 size={16} /></button></div></div>
    <div className="ld-strip ld-strip-4">
      <div className="ld-strip-item"><span>Fully paid</span><strong className="positive">{money(s.paidTotal)}</strong></div>
      <div className="ld-strip-item"><span>Balance due</span><strong className={s.unpaidTotal > 0 ? 'warning-text' : 'positive'}>{money(s.unpaidTotal)}</strong></div>
      <div className="ld-strip-item"><span>Days this month</span><strong>{s.monthDays}</strong></div>
      <div className="ld-strip-item"><span>Days this year</span><strong>{s.yearDays}</strong></div>
    </div>
    {pendingAdvanceTotal > 0 && <div className="ld-advance-pending"><HandCoins size={16} /> Advance pending: {money(pendingAdvanceTotal)}</div>}
    ...
    <div className="ld-days">{attendanceDays.map(([key, label]) => { const isPresent = record?.[key] ?? true; const dayInc = dayIncentiveOf(record, key); const daySite = daySiteOf(record, key); return <div className={`ld-day-card ${isPresent ? 'came' : 'absent'}`} key={key}><button className={`ld-day ${isPresent ? 'came' : 'absent'}`} onClick={() => void toggleDay(labourer, key)} disabled={settled}><span className="ld-day-label">{label}</span><span className="ld-day-state">{isPresent ? <><Check size={18} /> Came</> : <><X size={18} /> Absent</>}</span></button>{isPresent && !settled && <div className="ld-day-detail"><input type="number" inputMode="decimal" onFocus={selectZeroOnFocus} min="0" step="0.01" placeholder="Incentive" value={dayInc} onChange={(e) => void updateDayDetail(labourer, key, 'incentive', Number(e.target.value))} /><input type="text" placeholder="Site name" value={daySite} onChange={(e) => void updateDayDetail(labourer, key, 'site', e.target.value)} /></div>}{isPresent && settled && (dayInc > 0 || daySite) && <div className="ld-day-detail-static">{dayInc > 0 && <span>+{money(dayInc)}</span>}{daySite && <small>{daySite}</small>}</div>}</div>; })}</div>
    {weekPay ? (
      <div className="ld-salary-breakdown">
        <div className="ld-sb-row"><span>Earned this week</span><strong>{money(weekPay.earned)}</strong></div>
        {weekPay.toRecover > 0 && <div className="ld-sb-row ld-sb-recover"><span>Advance recovered</span><strong>−{money(weekPay.toRecover)}</strong></div>}
        <div className="ld-sb-divider" />
        <div className="ld-sb-row ld-sb-net"><span>Net payable this week</span><strong>{money(weekPay.netPayable)}</strong></div>
        {weekPay.carryForward > 0 && <div className="ld-sb-carry">{money(weekPay.carryForward)} will be recovered from next week's earnings</div>}
      </div>
    ) : <div className="ld-salary-box"><span>Salary to pay this week</span><strong>{money(finalAmount)}</strong></div>}
    <div className="ld-advance-action"><button className="button secondary" onClick={() => setShowAdvanceForm(!showAdvanceForm)}><HandCoins size={16} /> Give advance</button></div>
    ...
    {settled && linkedPayment ? (
      ...
      : <div className="ld-pay-panel">
        <div className="ld-pay-status">
          {isPartial ? <><span className="ld-status-label partial">Partial</span> Paid {money(paidSoFar)} of {money(linkedPayment.amount)} — balance {money(balance)} due</> : <span className="ld-status-label unpaid">Week saved — {money(balance)} not paid yet</span>}
        </div>
        {showPartial ? (
          <form className="ld-partial-form" onSubmit={(e) => { e.preventDefault(); if (payAmount > 0 && payAmount <= balance) { onRecordPayment(linkedPayment, payAmount); setShowPartial(false); setPayAmount(0); } }}>
            ...
            <button type="button" className="button secondary" onClick={() => { setShowPartial(false); setPayAmount(0); }}>Cancel</button>
          </form>
        ) : (
          <div className="ld-pay-buttons">
            <button className="button primary ld-pay-now" onClick={() => void onMarkPaid(linkedPayment)}>Pay full {money(balance)}</button>
            <button className="button secondary" onClick={() => { setShowPartial(true); setPayAmount(0); }}>Partial pay</button>
          </div>
        )}
      </div>
    ) : settled ? <div className="ld-mark-paid"><div className="ld-saved-notice">Week saved</div></div>
    : <div className="ld-actions">{weekPay && weekPay.netPayable <= 0 ? <><div className="ld-no-pay">Nothing to pay this week — advance covers all earnings</div><button className="button secondary ld-pay-now" onClick={() => void settleWeek(labourer, true)} disabled={present === 0}>Save week</button></> : <><button className="button primary ld-pay-now" onClick={() => void settleWeek(labourer, true)} disabled={present === 0}>Pay {money(weekPay?.netPayable ?? finalAmount)} now</button><button className="text-button ld-save-only" onClick={() => void settleWeek(labourer, false)}>Save without paying yet</button></>}</div>}
    <div className="ld-history"><div className="profile-section-heading"><h4>Advance history</h4><span>{pastAdvances.length} entries</span></div>{pastAdvances.length > 0 ? pastAdvances.map((adv) => { const recovered = Number(adv.amount) - Number(adv.amount_remaining); const canDelete = Number(adv.amount_remaining) === Number(adv.amount); return <div className="profile-payment-row advance-row" key={adv.id}><div><strong>{money(adv.amount)} · {dateLabel(adv.given_date)}</strong>{adv.note && <small>{adv.note}</small>}<small className="advance-recovery">Recovered {money(recovered)} · Pending {money(adv.amount_remaining)}</small></div><div className="profile-payment-actions">{canDelete ? <button className="icon-button danger" onClick={() => void onDeleteAdvance(adv)}><Trash2 size={15} /></button> : <AdvanceNoteEditor advance={adv} onSave={onEditAdvanceNote} />}</div></div>; }) : <div className="labour-empty">No advances given yet.</div>}</div>
    <div className="ld-history"><div className="profile-section-heading"><h4>Past weeks</h4><span>{pastPayments.length} entries</span></div>{pastPayments.length > 0 ? pastPayments.map((payment) => { const attRecord = labourer.attendance.find((r) => r.week_ending === payment.week_ending); const daysPresent = attRecord ? attendanceDays.filter(([key]) => attRecord[key]).length : null; const dayDetails = attRecord ? attendanceDays.filter(([key]) => attRecord[key]).map(([key, label]) => { const inc = dayIncentiveOf(attRecord, key); const site = daySiteOf(attRecord, key); return `${label}${inc > 0 ? ` +${money(inc)}` : ''}${site ? ` @ ${site}` : ''}`; }).join(' · ') : ''; const pStatus = payment.payment_status || (payment.paid ? 'Paid' : 'Unpaid'); const pBalance = Number(payment.balance ?? (payment.paid ? 0 : payment.amount)); const pPaid = Number(payment.amount_paid ?? (payment.paid ? payment.amount : 0)); const isPartialForThis = partialFor === payment.id; return <div className="profile-payment-row" key={payment.id}><div><strong>{weekRangeLabel(payment.week_ending)}</strong><small>{daysPresent !== null ? `${daysPresent}/7 days present` : (payment.notes || 'Weekly payment')}{dayDetails ? ` — ${dayDetails}` : ''}</small>{attRecord?.settled && pStatus !== 'Paid' && <button className="text-button tiny" onClick={() => void onUnlockWeek(attRecord)}>Make changes</button>}</div><div className="profile-payment-actions"><div className="pp-amounts"><strong>{money(payment.amount)}</strong>{pStatus === 'Partial' && <small className="pp-partial">Paid {money(pPaid)} · Bal {money(pBalance)}</small>}</div><span className={`pay-status-chip ${pStatus.toLowerCase()}`}>{pStatus === 'Paid' ? <><Check size={13} /> Full</> : pStatus === 'Partial' ? <><CircleDollarSign size={13} /> Partial</> : <><X size={13} /> Unpaid</>}</span>{pStatus !== 'Paid' && !isPartialForThis && <button className="button tiny primary" onClick={() => void onMarkPaid(payment)}>Pay full</button>}{pStatus !== 'Paid' && !isPartialForThis && <button className="button tiny secondary" onClick={() => { setPartialFor(payment.id); setPayAmount(0); }}>Partial</button>}{isPartialForThis && <form className="pp-partial-form" onSubmit={(e) => { e.preventDefault(); if (payAmount > 0 && payAmount <= pBalance) { onRecordPayment(payment, payAmount); setPartialFor(null); setPayAmount(0); } }}><input type="number" inputMode="decimal" onFocus={selectZeroOnFocus} min="1" max={pBalance} value={payAmount} onChange={(e) => setPayAmount(Number(e.target.value))} placeholder="Amount" required autoFocus /><button type="submit" className="button tiny primary">Pay {money(payAmount || 0)}</button><button type="button" className="button tiny secondary" onClick={() => { setPartialFor(null); setPayAmount(0); }}>Cancel</button></form>}<button className="button tiny secondary" onClick={() => downloadWeekExcel(labourer, payment.week_ending)} title="Download this week as Excel"><Download size={14} /> Excel</button><button className="icon-button danger" onClick={() => void onDeletePayment(payment)}><Trash2 size={15} /></button></div></div>; }) : <div className="labour-empty">No past weeks yet.</div>}</div>
  </div>;
}
...
function AdvanceForm({ labourer, onSubmit }: { labourer: LabourerWithPayments; onSubmit: (amount: number, date: string, note: string) => void }) {
  const [amount, setAmount] = useState(0);
  const [date, setDate] = useState(today);
  const [note, setNote] = useState('');
  return <form className="form-grid" onSubmit={(e) => { e.preventDefault(); if (amount > 0) onSubmit(amount, date, note); }}>
    <div className="ld-advance-intro">Give an advance to <strong>{labourer.name}</strong>. This will be automatically recovered from future weekly earnings.</div>
    <label>Amount *<input type="number" inputMode="decimal" onFocus={selectZeroOnFocus} min="1" step="0.01" value={amount} onChange={(e) => setAmount(Number(e.target.value))} required autoFocus /></label>
    <label>Date<input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></label>
    <label className="full">Note (optional)<input type="text" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Reason for advance" /></label>
    <div className="form-actions full"><button type="submit" className="button primary">Give {money(amount || 0)} advance</button></div>
  </form>;
}
